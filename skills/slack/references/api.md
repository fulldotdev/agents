...
For `chat.postMessage`, send one JSON object containing `channel` and `text`. Add `thread_ts` for a reply and `reply_broadcast: true` only when explicitly requested. Suppress rich previews with `unfurl_links: false` and `unfurl_media: false` unless the user explicitly wants them.
