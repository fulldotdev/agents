...
| com.fulldev.otis-restart (prepared, not installed) | first eligible day of each month, 05:00 | `~/.agents/global/scripts/otis-restart.py`; postpone to the next day when busy; report service recovery to Telegram System |
...
The monthly restart needs one administrator authorization on Otis. Run `bash ~/.agents/global/scripts/install-otis-restart.sh` in an interactive Otis terminal. It authorizes only `/sbin/shutdown -r now`, registers the 05:00 LaunchAgent, and attempts the approved restart test after checking for active work. The recovery check reuses the existing health checks. State lives in `~/.local/state/fulldev/restart/`; logs in `~/Library/Logs/fulldev/otis-restart.log`.
