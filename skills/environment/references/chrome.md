...
The browser preference is in `environment/SKILL.md`. This reference covers the integration details that differ by harness. Keep credentials and machine configuration out of the shared skills repository.
...
In T3, add `--chrome` to the Claude provider instance's launch arguments. Check that instance's configured home and account, since it can differ from standalone Claude. A fresh provider session may be needed to load the flag and browser tools. An installed extension is not proof that a T3 Claude session can control it; verify the connection there.
...
See [Otis operations](../../../operations/openclaw-otis.md) and https://docs.openclaw.ai/tools/chrome-extension.
