...
Choose one unique owner for this worker run, or use the owner and state file supplied by the scheduler. Keep it when resuming an interrupted run. `triage --incremental --compact --owner RUN --format yaml` persists the full batch before returning its compact view. `queue status --compact` reads it without recollecting; use repeated `--event ID` with `queue show` to batch full event reads. Recollect only when fresh evidence is needed, preserving the owner, batch ID, decisions and unresolved events. Pass a supplied `--state-file` to every queue command.
...
`work_index` covers the full Companies/Projects/Tasks index; `source_index` includes acknowledged context. Only unfinished `queue.events` require decisions. Read full events before deciding, including no-action decisions; titles and previews alone are insufficient. `queue context --lane companies|projects|tasks|gmail|slack|whatsapp|calendar|meetings|t3_threads --id ID` (repeatable) or `--query TEXT` reads matching full snapshot records. WhatsApp event reads also include the refreshed chat context, including outgoing messages. Missing matches do not prove that no destination exists: use the authoritative source when evidence is absent or stale. Plain `queue status` retains the legacy full output for diagnostics; avoid loading it routinely.
