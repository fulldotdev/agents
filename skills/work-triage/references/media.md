...
Collector downloads are per-run scratch files under `~/.cache/fulldev/work-triage/` by default (`WORK_TRIAGE_TEMP_DIR` can override it) and are not durable references. Old `.hermes` paths may still locate retained historical attachments; do not move or delete those as part of processing. Inspect the original when practical; create a smaller derivative or selected frames when needed by the analysis tool. Analysis derivatives never replace the original.
