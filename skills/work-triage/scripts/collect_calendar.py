...
def collect_account(account,a,b):
    lookahead_days=int(os.environ.get("CALENDAR_UPDATE_LOOKAHEAD_DAYS","90"))
    raw_events=fetch_events(account,a,b,MAX_ITEMS_PER_LANE)
    raw_events+=fetch_events(account,a,b+timedelta(days=lookahead_days),MAX_ITEMS_PER_LANE)
    items=[]; seen=set()
    for e in raw_events:
        ...
        if not event_in_window and not changed_in_window:
            continue
        seen.add(key)
        items.append({"id": e.get("id"), "ical_uid": e.get("iCalUID"), "title": e.get("summary"), "description": e.get("description"), "location": e.get("location"), "status": e.get("status"), "event_type": e.get("eventType"), "url": e.get("htmlLink"), "start": et(e.get("start")), "end": et(e.get("end")), "created": e.get("created"), "updated": e.get("updated"), "event_in_window": event_in_window, "changed_in_window": changed_in_window, "organizer": (e.get("organizer") or {}).get("email"), "creator": (e.get("creator") or {}).get("email"), "attendees": [x.get("email") for x in e.get("attendees") or [] if x.get("email")], "source_account": account})
        if len(items) >= MAX_ITEMS_PER_LANE:
            break
    return {"source": account, "ok": True, "items": items}
def main():
    p=argparse.ArgumentParser(); add_common_args(p); p.add_argument("--account",action="append"); args=p.parse_args(); a,b=window_from_args(args.after,args.before,require=True); r=base_result("calendar","window",a,b); r["sources"]=[]
