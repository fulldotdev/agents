...
Collector downloads are per-run scratch files under `~/.hermes/tmp/work-triage/` and are not durable references. For visual inspection, create a bounded derivative with a longest edge around 1600 px and target size below 500 KB. Use a contact sheet for related frames and a small crop when text detail is needed. Analysis derivatives never replace the original.

When media directly defines a requirement, decision, acceptance condition, blocker, handoff, or completion evidence, preserve the full-resolution original in Drive and add its durable URL to the owning Company or Project `Files` property. Retain the original source locator in the Task Timeline when the file is task evidence. Incidental media stays at its reopenable Gmail, Slack, WhatsApp, meeting, or file source.
