...
T3 on each machine connects to that machine's CLIProxyAPI pool on `127.0.0.1:8317`, described in the environment skill's [account-pool.md](../../environment/references/account-pool.md). Otis keeps working while MacBook is asleep. T3 Connect and SSH are separate from the provider proxy.
