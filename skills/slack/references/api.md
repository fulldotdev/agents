...
Set `SLACK_CONFIG_PATH` only when intentionally selecting one config file. If no workspace files exist, credentials fall back in this order:
...
Validate every configured user or bot token with `auth.test`. The display name and URL slug can differ; use `team_id` as the stable workspace identity. Before sending, select the exact workspace file rather than relying on the legacy fallback.
