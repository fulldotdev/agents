...
Update the Brief when the agreement changes. Preserve details that still apply. Omit what no source supports. Put project-wide context in the Project and link it instead of copying it into every Task.
...
Before appending, check whether T3, triage, or the user already saved the same fact. Compare meaning and dates, including later replies and corrections. Preserve old entries; correct a mistake with a new entry naming what it replaces, and update the Brief to the corrected facts.

Keep an existing `Timeline` as the Updates section rather than renaming it or creating a second log. Add a Brief to an existing Task only when the active work benefits from it, after checking the sources.
...
Use `ntn` and the Notion Markdown API. Read the current body immediately before editing, then use targeted `update_content` changes to the Brief and the end of Updates or Timeline. Preserve other sections and concurrent edits.
