...
- **No new Task is not no update.** Check whether explicit contact details belong on an existing Dex contact, new source context belongs in the owning Task Timeline or changes the project frame defined by `work-management`, or material files belong in Files. Use `dex-skill` for contact updates; preserve existing fields and resolve identity before writing. Do not create contacts or Tasks merely for group introductions or chatter.
...
- Existing work: use `work-management` for record selection, Task creation, and durable file routing; use `ntn` for Notion operations. Update only when new facts affect a decision.
...
Return only a numbered Markdown list with one short line per verified outcome. Name the record and action, such as `Company created`, `Task done`, `Project status changed`, `T3 started`, or `Draft created`. Show status changes as `old → new`, and failures with an evidence-backed fix. For browser failures, use [references/browser-troubleshooting.md](references/browser-troubleshooting.md). Report the verified cause, target machine, and required action; state when the remedy is unknown. Link every named Task, Project, or Company to Notion and every draft or T3 thread to its native URL. Record each reported action using the processing protocol.
