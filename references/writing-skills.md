...
Custom skills and `~/.agents/always.md` follow the plain style from `always.md`, plus these rules.
...
- Say each rule once. Style, permission, and evidence rules live in `~/.agents/always.md`, browser rules in `browser`. Do not restate them in a skill.
